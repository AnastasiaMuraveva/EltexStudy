...
void *thread_job(void *arg){
    ...
    while(1){
        ret = mq_receive(qid, q_msg, Q_MSG_SIZE, NULL);
        if(ret < 0){
        ...
        cli_addr.sin_addr.s_addr = atoi(q_msg);
        tmp = strchr(q_msg, ';');
        tmp++;
        cli_addr.sin_port = atoi(tmp);
        ...
        //sleep(1);
        ret = sendto(*fd, q_msg, MSG_SIZE, 0, (struct sockaddr *) &cli_addr, sizeof(cli_addr));
        if(ret < 0){
            perror("send error");
            pthread_exit(NULL);
        }
...
int main() {
    ...
    for (int i = 0; i < THREADS_NUM; i++) {
    	ret = pthread_create(&threads[i], NULL, thread_job, &sock);
    	if (ret < 0) {
            strcpy(q_msg, "exit");
            for(int j = 0; j < i; j++){
    ...
    while(flag){
        ...
        sprintf(q_msg, "%d;%d", cli_addr.sin_addr.s_addr, cli_addr.sin_port);
        ret = mq_send(qid, q_msg, Q_MSG_SIZE, 1);
        if(ret < 0){
